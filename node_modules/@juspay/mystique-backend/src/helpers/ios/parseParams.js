...
function transformKeys(config) {
  ...
  for (var i = 0; i<keys.length; i++) {
    if (typeof config[keys[i]] == "undefined" || config[keys[i]] == null) {
      ...
      if (keys[i] !== "id" && keys[i]!== "__filename" && keys[i]  !== "methods"  &&  keys[i]  !== "collectionView" && keys[i] !== "tableView") {
        delete config[keys[i]];
      }
    }
  }
...
function generateType(type){
  ...
  if (type=="linearLayout" || type=="relativeLayout") {
...
function changeKeys(config, type) {
  ...
  if (config.fontStyle) {
    font += config.fontStyle.indexOf("Regular") > -1 ? "normal," : "heavy,";
    delete config.fontStyle;
  }
...
module.exports = function(type, config, _getSetType) {
  ...
  if (config.centerInParent) {
    ...
    config.methods.push(self_alignToParent("centerHorizontal",{y: y}));
    config.methods.push(this_setCenter());
  } else if (config.centerVertical) {
    let _x = config.x || "0";
    let x = cS(_x*1 + (config.width*1)/2);
    config.methods.push(self_alignToParent("centerVertical",{x: x}));
    config.methods.push(this_setCenter());
    ...
    config.methods.push(self_alignToParent("bottom",{yOffset: fb, xOffset: fl}));
    config.methods.push(this_setCenter());
  ...
  return {config: transformKeys(config), type: generateType(type)};
}
